...
import { Search, Trash2, Edit, Plus, X, Save, Users, ChefHat, MessageSquare, Tags, ArrowLeft, Star, Clock, UtensilsCrossed, Headphones, Radio, CheckCircle, Circle, RefreshCw, Send, Menu, ChevronRight, LayoutDashboard, Upload, ImageIcon, Shield } from 'lucide-react';
...
import {
  getLiveChats, addLiveChatMessage, markLiveChatRead, closeLiveChat, seedDemoData,
...
type TabType = 'recipes' | 'users' | 'comments' | 'categories' | 'support';
...
export function AdminDashboard({ recipes, onUpdateRecipe, onDeleteRecipe, onAddRecipe, onBack }: AdminDashboardProps) {
  ...
  // Seed demo data and load from localStorage on mount
  useEffect(() => {
    seedDemoData();
    setLiveChats(getLiveChats());
  }, []);

  const refreshLiveChats = () => setLiveChats(getLiveChats());
  ...
  const [users, setUsers] = useState<User[]>([
    ...
    },
    {
      ...
      allergies: ['Dairy'],
      dietaryPreferences: ['Vegan', 'Low Sugar'],
    ...
    <div className="space-y-4">
      ...
      {viewingRecipe && (
        <RecipeDetailModal
          ...
          onDelete={(id: string) => {
            if (window.confirm(`Delete "${viewingRecipe.title}"?`)) {
              onDelete(id);
              setViewingRecipe(null);
            ...
            <div key={cat}>
              ...
              <div className="space-y-3">
                {(catRecipes as Recipe[]).map((recipe: Recipe) => (
                  <RecipeRow key={recipe.id} recipe={recipe} onView={() => setViewingRecipe(recipe)} onEdit={(e: React.MouseEvent) => { e.stopPropagation(); handleEdit(recipe); }} onDelete={(e: React.MouseEvent) => { e.stopPropagation(); if (window.confirm(`Delete "${recipe.title}"?`)) { onDelete(recipe.id); toast.success('Recipe deleted successfully'); }}} />
                ))}
              </div>
            </div>
          ))}
...
function RecipeForm({ formData, onChange }: any) {
  ...
  const handleIngredientsChange = (value: string) => {
    onChange({ ...formData, ingredients: value });
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        <label className="text-sm font-bold text-gray-700 ml-1">Ingredients (one per line)</label>
        <textarea
...
function LiveChatTab({ sessions, onRefresh }: { sessions: LiveChatSession[]; onRefresh: () => void }) {
  ...
  const handleSendReply = () => {
    ...
    onRefresh();
    const updated = getLiveChats().find(c => c.id === selectedSession.id);
