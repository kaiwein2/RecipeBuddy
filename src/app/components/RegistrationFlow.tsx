...
export function RegistrationFlow({ onComplete, onCancel, isLoading = false, initialStep = 1, initialData, isGoogleSSO = false }: RegistrationFlowProps) {
  ...
  const [data, setData] = useState<RegistrationData>({
    ...
    healthGoal: initialData?.healthGoal || [],
  });
        ...
        <AnimatePresence mode="wait">
          ...
          {step === 2 && (
            ...
            >
                ...
                <div className="flex flex-wrap gap-2">
                  {commonAllergies.map((allergy) => (
                    <Badge
                      ...
                      className={`cursor-pointer px-4 py-2 rounded-full transition-all ${data.allergies.includes(allergy)
              ...
              <div className="flex gap-3">
                <Button
                  onClick={() => isGoogleSSO ? onCancel() : setStep(1)}
                  variant="outline"
                ...
                <div className="flex flex-wrap gap-2">
                  {dietaryOptions.map((option) => (
                    <Badge
                      ...
                      className={`cursor-pointer px-4 py-2 rounded-full transition-all ${data.dietaryPreferences.includes(option)
          ...
          {step === 4 && (
            <motion.form
              onSubmit={(e) => {
                e.preventDefault();
                if (data.healthGoal.length > 0 && !isLoading) {
                  onComplete(data);
                }
              }}
              key="step4"
            ...
            >
                ...
                <div className="grid grid-cols-2 gap-3">
                  {healthGoals.map((goal) => (
                    <button
                      ...
                      className={`p-4 rounded-xl border-2 transition-all ${data.healthGoal.includes(goal)
              ...
              <div className="flex gap-3">
                ...
                <Button
                  type="submit"
                  disabled={data.healthGoal.length === 0 || isLoading}
                  className="flex-1 h-12 bg-gradient-to-r from-amber-500 to-orange-500 text-white rounded-xl disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                >
