...
import { X, ArrowRight, ArrowLeft, Plus, Minus, Upload, Calculator, ChevronDown, ChevronUp, Store, Droplet } from 'lucide-react';
...
export function AddRecipeForm({ onClose, onSubmit, initialData }: AddRecipeFormProps) {
  ...
  const isStep4Valid = formData.instructionEntries.some(e => e.text.trim().length > 0 && e.minutes !== '' && Number(e.minutes) > 0);
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-2 sm:p-4">
      <motion.div
        ...
        className="bg-white rounded-2xl sm:rounded-3xl shadow-2xl w-full max-w-2xl max-h-[95vh] sm:max-h-[90vh] overflow-hidden flex flex-col"
      >
        {/* Header */}
        <div className="bg-gradient-to-r from-amber-500 to-orange-500 px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between">
          <div>
            <h2 className="text-lg sm:text-xl font-bold text-white">{initialData ? 'Edit Recipe' : 'Add New Recipe'}</h2>
            <p className="text-amber-50 text-xs sm:text-sm">Step {step} of 5</p>
          </div>
          <button
            onClick={onClose}
                  ...
                  {formData.ingredientEntries.map((entry, index) => (
                    <div key={index} className="border border-gray-200 rounded-xl overflow-hidden">
                      ...
                      <div className="flex gap-2 p-3 bg-white items-center">
                        ...
                        <button
                          type="button"
                          onClick={() => updateIngredientField(index, 'type', entry.type === 'dry' ? 'liquid' : 'dry')}
                          className={`flex-shrink-0 flex items-center gap-1 text-xs font-medium px-2.5 py-1.5 rounded-lg border transition-colors ${
                            entry.type === 'liquid' 
                              ? 'bg-blue-50 text-blue-600 border-blue-200 hover:bg-blue-100' 
                              : 'bg-gray-50 text-gray-600 border-gray-200 hover:bg-gray-100'
                          }`}
                          title={`Currently ${entry.type}. Click to change.`}
                        >
                          <Droplet className={`w-3.5 h-3.5 ${entry.type === 'liquid' ? 'fill-blue-500' : ''}`} />
                          <span className="hidden sm:inline capitalize">{entry.type}</span>
                        </button>
                        <button
                          type="button"
              ...
              >
                ...
                <div className="bg-amber-50 border border-amber-200 rounded-xl p-3">
                  <p className="text-xs text-amber-800">
                    <strong>Tip:</strong> Describe each step clearly. Mention heat levels, visual cues, and common mistakes to avoid.
                  </p>
                </div>
                    ...
                    return (
                      <div key={index} className="border border-gray-200 rounded-xl overflow-hidden">
                        <div className="flex gap-2 p-3 bg-white items-start">
                          <div className="flex-shrink-0 w-7 h-7 sm:w-8 sm:h-8 bg-gradient-to-br from-amber-500 to-orange-500 text-white rounded-full flex items-center justify-center font-bold text-xs sm:text-sm mt-0.5">
                            {index + 1}
                          </div>
                          <div className="flex-1 min-w-0 space-y-2">
                            <Textarea
                              ...
                              className="w-full rounded-xl border-gray-200 min-h-[80px] sm:min-h-[100px] text-sm resize-none"
                            />
                            <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                              <div className="flex items-center gap-2 bg-gray-50 border border-gray-200 rounded-lg px-2 sm:px-3 py-1.5">
                                <label className="text-xs font-medium text-gray-600 whitespace-nowrap">Time *</label>
                                <Input
                                  type="number"
                                  min="1"
                                  value={entry.minutes}
                                  ...
                                  placeholder="0"
                                  className="w-14 sm:w-16 h-7 text-sm text-center border-gray-200 rounded-lg p-0 px-1"
                                />
                                <span className="text-xs text-gray-500">min</span>
                              </div>
                              <span className={`text-xs ${hasText && hasTime ? 'text-green-600' : hasText ? 'text-amber-600' : 'text-gray-400'}`}>
                                {!hasText
                                  ? 'Enter a description'
                                  : !hasTime
                                  ? 'Add time'
        ...
        {/* Footer Navigation */}
        <div className="border-t border-gray-200 px-4 sm:px-6 py-3 sm:py-4 flex gap-3">
          {step > 1 && (
            <Button
